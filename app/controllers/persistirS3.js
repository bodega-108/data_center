  ...
  try {
    ...
    if (ultimo_documento_id.idUltimoDocumento != 0) {
      ...
      respuesta.statusDesc =
        "Ha ocurrido un error al buscar el ultimo id de documento";
    }
  } catch (e) {
    /**Error*/
    console.log(e);
...
const migrateOroCommerce = async () => {
  ...
  try {
    ...
    for (let i = 0; i < listadoDocumentos.documentos.Items.length; i++) {
      ...
      console.log(
        "id documento: " + listadoDocumentos.documentos.Items[i].id_documento
      );
      console.log("id oc oro: " + listadoDocumentos.documentos.Items[i].oc_oro);
      const almacenado = await setOcOro(
        listadoDocumentos.documentos.Items[i].id_documento,
        parseInt(listadoDocumentos.documentos.Items[i].oc_oro)
      );
...
const asociarNv = async (id_documento, folio, mes, year, nv_sherpa) => {
  ...
  try {
    ...
    if (newsNVSH || newNVSS) {
      let notaVentaSherpa;
      let notaVentaSherpaByHistoryEta = await getETAInHistory(nv_sherpa);

      if (!notaVentaSherpaByHistoryEta.statusCod) {
        notaVentaSherpa = await getInfochina(nv_sherpa);
      } else {
        notaVentaSherpa = notaVentaSherpaByHistoryEta.nv;
        notaVentaSherpa.statusCod = true;
        const deleteNVETA = await deleteETAInHistory(nv_sherpa);
      }
      ...
      if (folio === 0 && notaVentaSherpa.data === "SIN NV SHERPA ASOCIADA") {
        ...
        folio !== 0 &&
        newNVSS &&
        notaVentaSherpa.data === "SIN NV SHERPA ASOCIADA"
      ) {
        respuesta.statusInfo =
          "No se registró nv sherpa, se actualiza nv softnet";
      } else if (
        folio !== 0 &&
        newNVSS &&
        notaVentaSherpa.data !== "SIN NV SHERPA ASOCIADA"
        ...
        respuesta.statusInfo =
          "Se registraron o actualizaron las notas de venta sherpa - emonk";
      } else if (
        !newNVSS &&
        notaVentaSherpa.data === "SIN NV SHERPA ASOCIADA"
      ) {
      ...
      if (
        detalleFacturaSoftnet.statusCod ||
        detalleFacturaSoftnet === "SIN NV ASOCIADA"
      ) {
        let params = {
          TableName: tablaDynamo,
          Item: {
            ...
            nv_softnet: detalleFacturaSoftnet.data
              ? dataNVSoftnet
              : "SIN NV ASOCIADA",
            nv_sherpa: notaVentaSherpa.statusCod
              ? dataNVSherpa
              : "SIN NV SHERPA ASOCIADA",
    ...
    Item: {
      ...
      nv_sherpa:
        idNVSherpa.length === 0
          ...
          : idNVSherpa.toString(),
      nv_softnet:
        idNVSoftnet === 0 ? "SIN NV ASOCIADA" : idNVSoftnet.toString(),
    },
  };
  ...
  if (listaOcBuild.listaOrdenes.length > listaAWS.documentos.Items.length) {
    ...
    for (let i = 0; i < listaOcBuild.listaOrdenes.length; i++) {
      ...
      if (idOroAWS.includes(id_sistema_oro)) {
        logger.warn(
          `NO PERSISTIMOS LA OC ${listaOcBuild.listaOrdenes[i].oc_oro.id_oc}`
        );
      } else {
        if (
          listaOcBuild.listaOrdenes[i].nv_sherpa === "SIN NV SHERPA ASOCIADA" ||
          listaOcBuild.listaOrdenes[i].nv_sherpa ===
            "No olvide ingresar Folio de Nota de Venta Valido<br>" ||
          listaOcBuild.listaOrdenes[i].nv_sherpa ===
            "Nota de Venta no registrada<br>"
        ) {
          logger.info(
            `EL DOCUMENTO ${listaOcBuild.listaOrdenes[i].id_documento} NO TIENE NV ASOCIADA`
          );
        } else {
          logger.info(listaOcBuild.listaOrdenes[i]);
          logger.info(
            `persistimos la oc ${listaOcBuild.listaOrdenes[i].oc_oro.id_oc}`
          );
          let id_oc = listaOcBuild.listaOrdenes[i].oc_oro.id_oc;
          let nv_sherpa_id =
            listaOcBuild.listaOrdenes[i].nv_sherpa[i].detalle[0].folio;
          let nv_sofnet = listaOcBuild.listaOrdenes[i].nv_sofnet;
          let id_documento =
            listaOcBuild.listaOrdenes[i].id_documento.toString();

          const perEta = await persistenciaSistemaEta(
            nv_sherpa_id,
            id_documento,
            id_oc,
            0
          ...
          if (perEta.statusCod) {
            logger.info(
              `INFO DOCUMENTO ${listaOcBuild.listaOrdenes[i].oc_oro.id_oc} ACTUALIZADO`
            );
          }
        }
      }
