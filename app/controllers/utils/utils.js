...
const newNVADD = async(documento, nv_sherpa)=>{
    ...
    if((documento.documentos.nv_sherpa !== '' || documento.documentos.nv_sherpa !== "SIN NV SHERPA ASOCIADA") && documento.documentos.nv_sherpa.length !== 0 ){
        
        for( let i = 0; i < documento.documentos.nv_sherpa.length; i++ ){
            let nvSherpaFromSoftnet = documento.documentos.nv_sherpa[i][0].detalle[0].folio;
...
const newNVADDSOFTNET = async(documento, nv_softnet)=>{
    ...
    if(documento.documentos.nv_softnet !== '' || documento.documentos.nv_softnet !== "SIN NV ASOCIADA" || documento.documentos.nv_softnet.length !== 0 ){
